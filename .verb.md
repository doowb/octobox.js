...
You can host your own instance of Octobox.js using Heroku. Heroku will ask you to provide a `GitHub Client ID` and `GitHub Client Secret` which you can get from GitHub by creating a new [OAuth Application](#github-oauth-application).

Optionally, you make set a Google Analytics Tracking ID that will place the Google Analytics script in each page.
...
You'll need a local PostgreSQL installed. On OSX you may use [Homebrew](http://mxcl.github.io/homebrew/) or the [PostgresApp](http://postgresapp.com). Please see these [further instructions for installing Postgres via Homebrew](http://www.mikeball.us/blog/setting-up-postgres-with-homebrew/).
...
The first file is `config.json` in the root of the project:
